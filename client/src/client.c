...
int main(void)
{
	...
	log_warning(logger, "Hola! Yo Soy warning");
...
*/
	...
	demas=config_get_string_value(config,"MALASANA");
/*
	log_info(logger,"Valor de ip: %s", ip);
	log_info(logger,"Valor de puerto: %s", puerto) ;
	log_info(logger,"Valor de valor: %s", valor) ;
	log_info(logger,"Valor de ELOTRO: %s", demas) ;
*/
...
void leer_consola(t_log* logger)
{
    ...
    while (1) {
        ...
        if (strlen(leido)==0) {
			log_warning(logger,"Entrada vacia") ;
			break;
        }
		if (leido) {
            add_history(leido);
